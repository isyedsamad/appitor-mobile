...
export default function MyAttendancePage() {
  ...
  const graphData = useMemo(() => {
    ...
    MONTHS.forEach((m, i) => {
      ...
      const p = data?.P || 0;
      const t = (data?.P || 0) + (data?.A || 0) + (data?.L || 0) + (data?.H || 0);
      ...
      <ScrollView>
        ...
        <View className="mt-4 px-5">
          {MONTHS.map((m, i) => {
            ...
            const key = getMonthKey(year, i < 9 ? i + 3 : i - 9);
            const data = sessionData?.months?.[key] || {};
            const total = (data.P || 0) + (data.A || 0) + (data.L || 0) + (data.H || 0);
            ...
            <View
              ...
              <View className="flex-row items-center justify-center gap-1">
                ...
                <View
                  ...
                  style={{
                    backgroundColor: colors.statusHbg,
                    borderWidth: 1,
                    borderColor: colors.statusHborder,
                  }}>
                  <AppText size="vmin" semibold style={{ color: colors.statusHtext }}>
                    Half Day
                  </AppText>
                </View>
                <View
                  className="flex-1 items-center whitespace-nowrap rounded-lg px-2 py-1"
                  style={{
                    backgroundColor: colors.statusObg,
                    borderWidth: 1,
                    borderColor: colors.statusOborder,
                  }}>
                  <AppText size="vmin" semibold style={{ color: colors.statusOtext }}>
                    Over Time
                  </AppText>
                </View>
              </View>
                    ...
                    status === 'P'
                      ...
                      : status === 'A'
                        ...
                        : status === 'L'
                          ...
                          : status === 'H'
                            ? colors.statusHborder
                            : status === 'O'
                              ? colors.statusOborder
                              : colors.border;
