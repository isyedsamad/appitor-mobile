...
import { Bell, CircleOff, Layers, User } from 'lucide-react-native';
...
export default function NotificationPage() {
  ...
  const [tab, setTab] = useState<'notice' | 'class' | 'personal'>(initialTab);
  ...
  useEffect(() => {
    ...
    async function loadAll() {
      ...
      try {
        ...
        const [nSnap, cSnap, mSnap] = await Promise.all([
          ...
          getDoc(
            doc(
              ...
              'items',
              'class_notices',
              `${schoolUser.className}_${schoolUser.section}_${schoolUser.currentSession}`
            )
          ),
          getDoc(
            doc(
        ...
        setClassNotices(sortByNewest(cSnap.exists() ? cSnap.data().items || [] : []));
  ...
  function handleTabChange(nextTab: 'notice' | 'class' | 'personal') {
    setTab(nextTab);
    clearBadgeForTab(nextTab);
  }

  function clearBadgeForTab(tab: 'notice' | 'class' | 'personal') {
  ...
  const data = useMemo(() => {
    ...
    if (tab === 'class') return classNotices;
    return messages;
  }, [tab, notices, classNotices, messages]);
  ...
  useEffect(() => {
    ...
    secureAxios.patch('/api/school/communication/student-message', {
      schoolId: schoolUser.schoolId,
      branch: schoolUser.currentBranch,
      studentUid: schoolUser.uid,
      ...
      <ScrollView>
        <View className="mx-5 mb-5 mt-6">
          <AppText size="body" semibold className="text-center">
            {tab === 'notice' ? 'Noticeboard' : tab === 'class' ? 'Class Notices' : 'Student Inbox'}
          </AppText>
          <AppText size="subtext" muted className="text-center">
            {tab === 'notice'
              ? 'School-wide announcements'
              : tab === 'class'
                ? 'Updates for your class & section'
                  ...
                  <View
                    ...
                    <View className="flex-1 flex-row items-center">
                      <View
                        ...
                        {tab === 'class' && <Layers size={16} color={colors.primary} />}
